...
fn test_mint_asset_when_new_asset_definition_created() -> Result<()> {
    ...
    let tea_definition_id = AssetDefinitionId::new("tea", "wonderland")?;
    let register_tea_definition = RegisterBox::new(IdentifiableBox::from(
        AssetDefinition::new_quantity(tea_definition_id),
    ));
