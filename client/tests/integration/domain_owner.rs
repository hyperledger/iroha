...
fn domain_owner_domain_permissions() -> Result<()> {
    ...
    test_client.submit_blocking(SetKeyValueBox::new(kingdom_id.clone(), key.clone(), value))?;
    test_client.submit_blocking(RemoveKeyValueBox::new(kingdom_id.clone(), key))?;
    ...
    test_client.submit_blocking(GrantBox::new(token.clone(), bob_id.clone()))?;
    test_client.submit_blocking(RevokeBox::new(token, bob_id))?;

    // check that "alice@wonderland" as owner of domain can unregister her domain
    test_client.submit_blocking(UnregisterBox::new(kingdom_id))?;
...
fn domain_owner_account_permissions() -> Result<()> {
    ...
    test_client.submit_blocking(RegisterBox::new(mad_hatter))?;
    ...
    test_client.submit_blocking(BurnBox::new(
        mad_hatter_new_keypair.public_key().clone(),
        mad_hatter_id.clone(),
    ))?;
    ...
    test_client.submit_blocking(MintBox::new(
        SignatureCheckCondition::new(true.into()),
        mad_hatter_id.clone(),
    ))?;
    ...
    let key: Name = "key".parse()?;
    let value: Name = "value".parse()?;
    test_client.submit_blocking(SetKeyValueBox::new(
        mad_hatter_id.clone(),
        key.clone(),
        value,
    ))?;
    test_client.submit_blocking(RemoveKeyValueBox::new(mad_hatter_id.clone(), key))?;
    ...
    test_client.submit_blocking(UnregisterBox::new(mad_hatter_id))?;
...
fn domain_owner_asset_definition_permissions() -> Result<()> {
    ...
    test_client.submit_blocking(RegisterBox::new(rabbit))?;
    ...
    let transaction = TransactionBuilder::new(bob_id.clone())
        .with_instructions([RegisterBox::new(coin)])
        .sign(bob_keypair)?;
    ...
    test_client.submit_blocking(TransferBox::new(bob_id, coin_id.clone(), rabbit_id))?;

    // check that "alice@wonderland" as owner of domain can edit metadata of asset definition in her domain
    let key: Name = "key".parse()?;
    let value: Name = "value".parse()?;
    test_client.submit_blocking(SetKeyValueBox::new(coin_id.clone(), key.clone(), value))?;
    test_client.submit_blocking(RemoveKeyValueBox::new(coin_id.clone(), key))?;
    ...
    test_client.submit_blocking(UnregisterBox::new(coin_id))?;
...
fn domain_owner_asset_permissions() -> Result<()> {
    ...
    let transaction = TransactionBuilder::new(bob_id.clone())
        .with_instructions([RegisterBox::new(coin), RegisterBox::new(store)])
        .sign(bob_keypair)?;
    test_client.submit_transaction_blocking(&transaction)?;
    ...
    test_client.submit_blocking(RegisterBox::new(bob_coin))?;
    test_client.submit_blocking(UnregisterBox::new(bob_coin_id.clone()))?;

    // check that "alice@wonderland" as owner of domain can burn, mint and transfer assets in her domain
    test_client.submit_blocking(MintBox::new(10u32.to_value(), bob_coin_id.clone()))?;
    test_client.submit_blocking(BurnBox::new(5u32.to_value(), bob_coin_id.clone()))?;
    test_client.submit_blocking(TransferBox::new(bob_coin_id, 5u32.to_value(), alice_id))?;
    ...
    test_client.submit_blocking(RemoveKeyValueBox::new(bob_store_id.clone(), key))?;
...
fn domain_owner_trigger_permissions() -> Result<()> {
    ...
    let trigger_instructions = vec![MintBox::new(1_u32, asset_id)];
    ...
    test_client.submit_blocking(MintBox::new(1_u32, trigger_id.clone()))?;
    test_client.submit_blocking(BurnBox::new(1_u32, trigger_id.clone()))?;
    ...
    test_client.submit_blocking(UnregisterBox::new(trigger_id))?;
