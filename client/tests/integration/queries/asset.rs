...
fn find_asset_total_quantity() -> Result<()> {
    ...
    let initial_total_asset_quantity = test_client.query(
    ...
    let result = test_client.query(FindTotalAssetQuantityByAssetDefinitionId::new(
        definition_id.clone(),
    ))?;
    ...
    let total_asset_quantity = test_client.query(
        FindTotalAssetQuantityByAssetDefinitionId::new(definition_id.clone()),
    )?;
