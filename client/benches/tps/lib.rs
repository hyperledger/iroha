...
impl Config {
    ...
    pub fn measure(self) -> Result<Tps> {
        ...
        thread::sleep(core::time::Duration::from_secs(7));
...
impl MeasurerUnit {
    ...
    fn ready(self) -> Result<Self> {
        ...
        let burn_permission: PermissionToken = CanBurnUserAssets::new(asset_id.clone()).into();
        let grant_burn_permission_to_alice =
            GrantBox::new(burn_permission, alice_id.clone()).into();
        let transfer_permission: PermissionToken =
            CanTransferUserAssets::new(asset_id.clone()).into();
        let grant_transfer_permission_to_alice =
            GrantBox::new(transfer_permission, alice_id).into();
        let grant_tx = Transaction::new(
            account_id,
            Executable::Instructions(vec![
                grant_burn_permission_to_alice,
                grant_transfer_permission_to_alice,
            ]),
            100_000,
        )
        .sign(keypair)?;
