...
def test_api_version_responce_presence(
    ...
    with allure.step("THEN the response should have a information"):
...
def test_api_version_response_pattern(GIVEN_get_request_to_api_version_enpoint_is_sent):
    ...
    with allure.step(
        ...
        assert re.match(r"^\d+", response), "Version is missing or in incorrect format"
...
def test_api_version_response_format(GIVEN_get_request_to_api_version_enpoint_is_sent):
    with allure.step("WHEN I get the response"):
        response = GIVEN_get_request_to_api_version_enpoint_is_sent
    with allure.step("THEN the response should be plain text"):
        assert response, "Response is not a valid plain text"
...
def test_api_version_response_content_length(
    ...
    with allure.step("WHEN I get the response"):
        response = GIVEN_get_request_to_api_version_enpoint_is_sent
    with allure.step("THEN the content length should be 1 byte"):
        assert len(response.content) == 1, "Response content is 1 byte"
