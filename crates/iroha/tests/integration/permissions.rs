...
use iroha_executor_data_model::permission::{
    asset::{CanMintAssetWithDefinition, CanTransferUserAsset},
...
#[test]
fn different_privileges_when_same_permission_with_different_payload() {
...
fn stored_vs_granted_permission_payload() -> Result<()> {
    ...
    let allow_alice_to_mint_mouse_asset = Grant::account_permission(
        Permission::new("CanMintUserAsset".parse().unwrap(), value_json),
        alice_id,
    );
