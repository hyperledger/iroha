...
impl<G: GenesisNetworkTrait, W: WorldTrait> Handler<Gossip> for Sumeragi<G, W> {
    ...
    async fn handle(&mut self, Gossip: Gossip) {
        ...
        let txs = if let Some(block) = self.voting_block.read().await.clone() {
            txs.into_iter().filter(|tx| !block.contains(tx)).collect()
        } else {
            txs
        };
...
impl<G: GenesisNetworkTrait, W: WorldTrait> Sumeragi<G, W> {
    ...
    async fn gossip_msg(&self, msg: impl Into<Message> + Send) {
        let msg = VersionedMessage::from(msg.into());
        self.topology
            .sorted_peers()
            .iter()
    ...
    async fn gossip_msgs(&self, msgs: impl IntoIterator<Item = impl Into<Message>> + Send) {
        let msgs = msgs
            .into_iter()
            .map(Into::into)
    ...
    pub async fn validate_and_publish_created_block(&mut self, block: ChainedBlock) -> Result<()> {
        ...
        for event in Vec::<Event>::from(&block) {
            ...
            drop(self.events_sender.broadcast(event).await);
        }
        ...
        *self.voting_block.write().await = Some(voting_block.clone());
        self.gossip_msg(BlockCreated::from(block.sign(&self.key_pair)?))
            .await;
...
impl VotingBlock {
    ...
    pub fn contains(&self, tx: impl Borrow<VersionedAcceptedTransaction>) -> bool {
        self.block.as_inner_v1().contains(tx)
    }
}
    ...
    impl BlockCreated {
        ...
        pub async fn handle<G: GenesisNetworkTrait, W: WorldTrait>(
            ...
            if sumeragi.voting_block.write().await.is_some()
                || sumeragi.wsv.recently_committed(&self.block)
            {
                return Ok(());
            }
            ...
            for event in Vec::<Event>::from(&self.block) {
                iroha_logger::info!(?event, "Event happened");
                drop(sumeragi.events_sender.broadcast(event).await);
            }
    ...
    impl TransactionForwarded {
        ...
        pub async fn handle<G: GenesisNetworkTrait, W: WorldTrait>(
                ...
                .push(self.transaction.clone(), &*sumeragi.wsv)
            {
                ...
                Err(queue::Error::InBlockchain(_)) | Ok(()) => Ok(()),
                Err(err) => Err(err).wrap_err("Failed to push tx in queue"),
            }
        }
    }
