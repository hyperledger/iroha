...
fn main() -> color_eyre::Result<()> {
    ...
    if std::env::args().any(|a| cli::is_genesis(&a)) {
        writeln!(
            output,
            "{}",
...
mod cli {
    ...
    pub fn is_docs(arg: &str) -> bool {
        ["--docs", "-d"].contains(&arg)
    }

    pub fn is_schema(arg: &str) -> bool {
        ["--schema", "-s"].contains(&arg)
    }

    pub fn is_genesis(arg: &str) -> bool {
