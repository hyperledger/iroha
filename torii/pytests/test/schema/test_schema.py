...
@allure.id("1422")
def test_schema_response_json_format(
        GIVEN_get_request_to_schema_endpoint_is_sent):
...
@allure.id("1421")
def test_schema_response_content_type(
        GIVEN_get_request_to_schema_endpoint_is_sent):
    with allure.step("WHEN I send GET request to /schema"):
        response = GIVEN_get_request_to_schema_endpoint_is_sent
...
@allure.id("1424")
def test_schema_response_json_schema(
        GIVEN_get_request_to_schema_endpoint_is_sent):
    schema_file_path = Path(__file__).parents[2] / "common" / "schemas" / "get_schema_response.json"
