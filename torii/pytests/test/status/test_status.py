...
@allure.id("1454")
def test_status_response_json_format(
        GIVEN_get_request_to_status_endpoint_is_sent):
    with allure.step(
...
def test_status_request_with_unexpected_header(
    ...
    with allure.step(
            "WHEN I send GET request to /status with unexpected header"):
...
@allure.id("1456")
def test_status_response_content_type(
        GIVEN_get_request_to_status_endpoint_is_sent):
    with allure.step(
            "WHEN I send GET request to /status"):
        response = GIVEN_get_request_to_status_endpoint_is_sent
...
@allure.id("1457")
def test_status_response_json_schema(
        GIVEN_get_request_to_status_endpoint_is_sent):
    schema_file_path = Path(__file__).parents[2] / "common" / "schemas" / "get_status_response.json"
    ...
    with allure.step(
            "THEN the response JSON should match the expected schema"):
        try:
            validate(instance=response, schema=schema)
        except ValidationError as ve:
            assert False, \
                f"Response JSON does not match the expected schema: {ve}"
