...
def test_post_configuration_invalid_data():
    with allure.step("WHEN I send POST request with invalid data to /configuration"):
        response = requests.post(f"{BASE_URL}/configuration", data=json.dumps({"logger": {"level": 'invalid'}}))
...
def test_post_configuration_valid_logger_level(log_level):
    with allure.step(f"WHEN I send POST request to /configuration with logger level {log_level}"):
        requests.post(f"{BASE_URL}/configuration", data=json.dumps({"logger": {"level": log_level}}))

    with allure.step(f"THEN the log level should be {log_level}"):
        get_response = requests.get(f"{BASE_URL}/configuration")
        assert get_response.json()["logger"]["level"] == log_level, \
