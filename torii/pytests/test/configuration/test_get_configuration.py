...
def test_configuration_response_json_format(
    ...
    with allure.step(
            "WHEN I send GET request to /configuration"):
...
def test_configuration_request_with_unexpected_param(
    ...
    with allure.step(
            "WHEN I send GET request to /configuration"):
        response = GIVEN_get_request_with_unexpected_param_to_configuration_enpoint_is_sent
    with allure.step(
            "THEN the response should be in JSON format"):
        assert response.json(), \
            "Response is not a valid JSON object"
...
def test_configuration_response_json_schema(
        GIVEN_get_request_to_configuration_endpoint_is_sent):
    schema_file_path = Path(__file__).parents[2] / "common" / "schemas" / "get_configuration_response_schema.json"
...
def test_configuration_response_logger_level(
    ...
    with allure.step("THEN the 'logger.level' should be one of the valid log levels"):
        assert response["logger"]["level"] in valid_log_levels, \
...
def test_configuration_response_content_length(
    ...
    with allure.step("THEN the content length should be within the expected range 27-28 bytes"):
        content_length = len(response.content)
        assert 27 <= content_length <= 28, \
