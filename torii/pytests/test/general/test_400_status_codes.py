...
def test_method_not_allowed():
    with allure.step(
            "WHEN I send GET request to /method_not_allowed"):
        response = requests.get(f"{BASE_URL}/method_not_allowed")
    with allure.step(
            "THEN the response status code should be 405"):
        assert response.status_code == 405, \
            f"Status code is not 405 for /method_not_allowed"
...
def test_request_uri_too_long():
    with allure.step(
            "WHEN I send an oversized GET request to /metrics"):
        response = requests.get(f"{BASE_URL}/metrics", params={"long_param": "a" * 65515})
    with allure.step(
            "THEN the response status code should be 414 (Request-URI Too Long)"):
        assert response.status_code == 414, \
            f"Status code is not 414"
