...
def test_post_configuration_logger_level(log_level):
    with allure.step(f"WHEN I send POST request to /configuration with logger level {log_level}"):
        response = requests.post(f"{BASE_URL}/configuration", data=json.dumps({"logger": {"level": log_level}}))
...
def test_api_version_status_code_200():
    with allure.step(
            f"WHEN I send GET request to /api_version"):
        response = requests.get(f"{BASE_URL}/api_version")
    with (allure.step(
            "THEN the response status code should be 200")):
        assert response.status_code == 200, \
            f"Status code is not 200 for /api_version"
...
def test_configuration_status_code_200():
    ...
    with allure.step("THEN the response status code should be 200"):
        assert response.status_code == 200, \
            "Status code is not 200 for /configuration"
...
def test_health_status_code_200():
    ...
    with allure.step("THEN the response status code should be 200"):
        assert response.status_code == 200, \
            "Status code is not 200 for /health"
...
def test_schema_status_code_200():
    ...
    with allure.step("THEN the response status code should be 200"):
        assert response.status_code == 200, \
            "Status code is not 200 for /schema"
...
def test_status_status_code_200():
    ...
    with allure.step("THEN the response status code should be 200"):
        assert response.status_code == 200, \
            "Status code is not 200 for /status"
...
def test_metrics_status_code_200():
    ...
    with allure.step("THEN the response status code should be 200"):
        assert response.status_code == 200, \
            "Status code is not 200 for /metrics"
...
def test_api_version_status_code_200_with_unexpected_param():
    with allure.step(
            f"WHEN I send GET request to /api_version with an unexpected parameter"):
        response = requests.get(f"{BASE_URL}/api_version", params={"unexpected": "param"})
    with allure.step(
            "THEN the response status code should be 200"):
        assert response.status_code == 200, \
            f"Status code is not 200 for /api_version with unexpected parameter"
...
def test_configuration_status_code_200_with_unexpected_param():
    with allure.step("WHEN I send GET request to /configuration with an unexpected parameter"):
        response = requests.get(f"{BASE_URL}/configuration", params={"unexpected": "param"})
    with allure.step("THEN the response status code should be 200"):
        assert response.status_code == 200, \
...
def test_metrics_status_code_200_with_unexpected_param():
    with allure.step("WHEN I send GET request to /metrics with an unexpected parameter"):
