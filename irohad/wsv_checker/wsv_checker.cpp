...
template<class C>
auto
operator<<(std::ostream& os, C const& coll) -> std::enable_if_t<is_container<C>::value, std::ostream&>
{
   os << "[";
   auto comma = "";
...
namespace {
std::string
tolower(std::string_view src)
{
   std::string dst;
   dst.reserve(src.size());
   for (auto const c : src)
      dst += (char)std::tolower(c);
   return dst;
}
std::string&
tolower(std::string& srcdst)
{
   for (auto& c : srcdst)
...
constexpr auto
tolower(CharT const (&str)[sz])
{
   std::array<CharT, sz> ret;
   const CharT* p = str;
   while (*p)
      ret[p - str] = std::tolower(*p++);
   return ret;
}
}   // namespace
...
DEFINE_string(pg_opt,
              "dbname=iroha_default host=localhost port=5432 user=postgres password=postgres",
...
{
   return std::make_shared<logger::LoggerManagerTree>(
     logger::LoggerConfig{logger::LogLevel::kInfo, logger::getDefaultLogPatterns()});
}
...
try {
   ...
   IROHA_EXPECTED_TRY_GET_VALUE(
     ...
     PgConnectionInit::init(
       ...
       PostgresOptions(FLAGS_pg_opt, "iroha_default", log_manager->getChild("PostgresOptions")->getLogger()),
       log_manager));
   pg_pool_wrapper_ = std::move(pool_wrapper);

   IROHA_EXPECTED_TRY_GET_VALUE(
     rdb_port, RdbConnectionInit::init(StartupWsvDataPolicy::kReuse, RocksDbOptions{FLAGS_rocksdb_path}, log_manager));
   db_context_ = std::make_shared<ametsuchi::RocksDBContext>(std::move(rdb_port));
...
#define STRX(x) #x
#define STR(x) STRX(x)
#define PRINT_NAME_HAVE_DIFFERENT(name, x, y) fmt::print(name " have different " STR(x) ": '{}' and '{}'\n", (x), (y))
...
#define CHECK_EQUALS_STR(type, name, x, y)                                                                             \
   if ((x) != (y)) {                                                                                                   \
      TYPES_PRINT_NAME_HAVE_DIFFERENT(type, (name), short_string(x), short_string(y));                                 \
      checked_result = false;                                                                                          \
   }
#define CHECK_EQUALS_JSON(type, name, x, y)                                                                            \
   if ((x) != (y)) {                                                                                                   \
      auto xd = (x).dump();                                                                                            \
      auto yd = (y).dump();                                                                                            \
      TYPES_PRINT_NAME_HAVE_DIFFERENT(type, (name), short_string(xd), short_string(yd));                               \
      checked_result = false;                                                                                          \
   }
#define CHECK_EQUAL_RANGES(type, x, y)                                                                                 \
   if (not xequal((x), (y), [](auto const& l, auto const& r) { return l.check_equals(r); })) {                         \
...
{
   using namespace std;
   return std::equal(begin(forward<C1>(c1)), end(forward<C1>(c1)), begin(forward<C2>(c2)), end(forward<C2>(c2)));
}
template<class C1, class C2, class F>
auto
xequal(C1&& c1, C2&& c2, F&& pred)
{
   using namespace std;
   return std::equal(
     forward<C1>(c1).begin(), forward<C1>(c1).end(), forward<C2>(c2).begin(), forward<C2>(c2).end(), forward<F>(pred));
}
...
struct short_string
{
   ...
   friend std::ostream& operator<<(std::ostream& os, short_string const& s) { return os << s.sstr << s.dots; }
};
template<>
...
std::string_view
get_unquoted_key(string_view& key)
{
...
inline void
unquote(std::basic_string<_CharT, _Traits, _Alloc>& data)
{
...
struct Peer
{
   ...
   bool operator<(Peer const& p) const { return pubkey < p.pubkey; }
   friend ostream& operator<<(ostream& os, Peer const& p)
   {
      return os << "\n  "
                << "pubkey:" << p.pubkey << " address:" << p.address << " tls:" << p.tls;
   }
...
struct Role
{
   ...
   bool operator<(Role const& o) const { return name < o.name; }
...
struct AssetPrecision
{
   ...
   bool operator<(AssetPrecision const& o) const { return name < o.name; }
   friend ostream& operator<<(ostream& os, AssetPrecision const& a) { return os << a.name << ":" << a.precision; }
...
struct AssetQuantity
{
   ...
   bool operator<(AssetQuantity const& o) const { return name < o.name; }
   friend ostream& operator<<(ostream& os, AssetQuantity const& a) { return os << a.name << ":" << a.quantity; }
   static AssetQuantity from_key_value(string_view& key, string_view val)
   {
      auto name = get_unquoted_key(key);
      assert(key.empty());
      return AssetQuantity{std::string(name), std::stod(std::string(val))};   // maybe stold
   }
...
struct GrantablePermissions
{
   ...
   bool operator==(GrantablePermissions const& o) const
   {
      return permittee_account_id == o.permittee_account_id and permission_bits == o.permission_bits;
   }
   bool operator!=(GrantablePermissions const& o) const
   {
      return permittee_account_id != o.permittee_account_id and permission_bits != o.permission_bits;
   ...
   int count_inequalities(GrantablePermissions const& o) const
   {
      ...
      COUNT_INEQUALITIES("GrantablePermissions", "", permittee_account_id, o.permittee_account_id)
      COUNT_INEQUALITIES("GrantablePermissions", "", permission_bits, o.permission_bits)
      return inequalities_counter;
   }
   friend std::ostream& operator<<(std::ostream& os, const GrantablePermissions& gp)
   {
...
struct fmt::formatter<GrantablePermissions>
{
   ...
   {
      return format_to(ctx.out(), "{{{}:{}}}", gp.permittee_account_id, gp.permission_bits);
   }
};
...
struct Account
{
   ...
   bool operator<(Account const& o) const { return name < o.name; }
   friend ostream& operator<<(ostream& os, Account const& a)
   {
      ...
      return os << "\n    " << a.name << ":"
                ...
                << "\n     quorum:" << a.quorum
                << "\n     assets[" << a.assetsquantity.size() << "]:[" << a.assetsquantity << "]"
                << "\n     roles[" << a.roles.size() << "]:[" << a.roles << "]"
                << "\n     grantable_permissions[" << a.grantable_permissions.size() << "]:[" << a.grantable_permissions << "]"
                << "\n     signatories[" << a.signatories.size() << "]:[" << a.signatories << "]";
      // clang-format on
   }
   ...
   bool check_equals(Account const& o) const
   {
      ...
      if (assetsquantity.size() != o.assetsquantity.size()) {
         fmt::print("Accounts have different sizes of assetsquantity: '{}' and '{}'\n",
                    assetsquantity.size(),
      ...
      if (not xequal(
            assetsquantity, o.assetsquantity, [](auto const& l, auto const& r) { return l.check_equals(r); })) {
         fmt::print("Accounts have different assetsquantity\n");
         ++inequalities_counter;
         return false;
      }
      if (not xequal(signatories, o.signatories, [&](auto const& l, auto const& r) {
             if (l != r) {
                fmt::print("Accounts '{}' have different signatories '{}' and '{}'\n", name, l, r);
                return false;
             }
             return true;
          })) {
      ...
      if (not xequal(grantable_permissions, o.grantable_permissions, [&](auto const& l, auto const& r) {
             if (l != r) {
                fmt::print("Accounts '{}' have different grantable_permissions '{}' and '{}'\n", name, l, r);
                return false;
             }
             return true;
          })) {
         fmt::print("Accounts '{}' have different grantable_permissions: sizes are {} and {}\n",
                    name,
...
struct Domain
{
   ...
   bool operator<(Domain const& o) const { return name < o.name; }
   ...
   bool check_equals(Domain const& other) const
   {
      ...
      if (default_role != other.default_role) {
         cout << "Domain default_role differ: '" << default_role << "' vs '" << other.default_role << "'";
         return false;
      }
      if (not std::equal(begin(accounts),
                         end(accounts),
      ...
      if (not std::equal(begin(assets_precision),
...
struct Wsv
{
   ...
   [[nodiscard]] bool check_equals(Wsv const& other) const
   {
      ...
      CHECK_EQUALS("Wsv-s", total_transactions_count, other.total_transactions_count)
   ...
   {
      Domain dom_to_find;
      dom_to_find.name = std::move(domain_id);   // fixme do not copy, find by const&
   ...
   Account const& find_account_by_id(std::string account_id)
   {
      ...
      Account acc_to_find;
      acc_to_find.name = std::move(account_id);   // fixme do not copy, find by const&
...
bool
Wsv::from_rocksdb(RocksDbCommon& rdbc)
{
   std::map<std::string, std::set<GrantablePermissions>>
     grant_perms_map;   // required because permittee domain could not exist at the moment of iteration
   ...
   auto status = rdbc.enumerate(
     [&](auto const& it, size_t key_sz) {
        ...
        auto key_starts_with_and_drop = [&key](string_view sv) {
           if (sv.size() > key.size())
              return false;
           //           int* p_key = static_cast<int*>(key.data());
           //           int* p_sv = static_cast<int*>(sv.data());
           //           auto* p_sv_end = sv.data() + sv.size();
           //           while ((p_sv_end - static_cast<char*>(p_sv) >= sizeof(*p_sv)) and *p_sv++ == *p_key++)
           //              ;
           //           check last bytes
           auto it_key = begin(key);
           auto it_sv = begin(sv);
           while (it_sv != end(sv) and *it_key == *it_sv)
              ++it_key, ++it_sv;
           if (it_sv == end(sv)) {
        ...
        if (key_starts_with_and_drop(RDB_F_VERSION)) {
           ...
           assert(schema_version == "1#2#0" &&
                  "This version of wsv_checker can check WSV in RocksDB of version 1.2.0 only");
           ...
           if (key_starts_with_and_drop(RDB_PEERS)) {
              if (key_starts_with_and_drop(RDB_ADDRESS)) {
                 ...
                 assert(key.empty());
                 auto [it_peer, inserted_peer] = peers.insert(Peer{std::string(pubkey), std::string(val), ""});
           ...
           auto [it_r, inserted_r] = roles.emplace(Role::from_key_value(key, val));
           assert(key.empty());
           ...
           if (key_starts_with_and_drop(RDB_F_TOTAL_COUNT)) {
              ...
              auto [it_dom, inserted_dom] = domains.insert(Domain{std::string(domname), std::string(val), {}, {}});
              auto& dom = *it_dom;
              if (key.empty()) {
                 assert(inserted_dom);
              } else {
                 if (key_starts_with_and_drop(RDB_ACCOUNTS)) {
                    ...
                    if (key_starts_with_and_drop(RDB_ASSETS)) {
                       auto [it_aq, inserted_ass] = acc.assetsquantity.emplace(AssetQuantity::from_key_value(key, val));
                       assert(key.empty());
                       ...
                       assert(key.empty());
                       auto [it_s, inserted] = acc.signatories.emplace(tolower(signame));
                       ...
                       auto [it_r, inserted_r] = acc.roles.insert(std::string(rolename));
                       assert(inserted_r && "Role was not inserted");
                       ...
                       } else if (key_starts_with_and_drop(RDB_F_TOTAL_COUNT)) {   // kAccountDetailsCount
                       ...
                       *jso = unquoted_val;
                       ...
                       assert(key.empty());
                       auto peracc_id = (std::string(permittee_acc));   // += "@") += permittee_dom;
           ...
           if (key_starts_with_and_drop(RDB_F_TOTAL_COUNT)) {
              total_transactions_count = std::stoull(std::string(val));   // fixme do not create string
           } else if (key_starts_with_and_drop(RDB_ACCOUNTS)) {
              auto accid = get_unquoted_key(key);
              (void)accid;
              // auto& acc = find_account_by_id(std::string(accid));
              if (key_starts_with_and_drop(RDB_F_TOTAL_COUNT)) {
                 // TODO select count(*) from tx_positions where creator_id='did@identity' and asset_id is NULL
                 key = {};
              ...
              assert(0 and "Wrong DB layout - unexpected key under RDB_TRANSACTIONS");
           }
        }   // RDB_TRANSACTIONS
           ...
           assert(0 && "unexpected key under RDB_ROOT RDB_WSV '" RDB_ROOT RDB_WSV "'");
        }
        assert(key.empty());
        return true;
     },
   ...
   for (auto& [acc, detcnt] : details_count) {
      size_t wrcnt = 0;
      for (auto& writer : acc->details_json)
         wrcnt += writer.size();
      if (wrcnt != detcnt) {
         cout << "acc->name" << acc->name << endl;
         cout << acc->details_json.dump(2) << endl;
         cout << "acc->details_json[writer].size():" << wrcnt << " detcnt:" << detcnt << endl;
      }
      assert(wrcnt == detcnt);
   }
...
bool
Wsv::from_postgres(soci::session& sql)
{
   using namespace soci;

   // clang-format off
   struct{std::string ma, mi, pa;} version;
   sql << "SELECT iroha_major,iroha_minor,iroha_patch FROM schema_version",
     into(version.ma), into(version.mi), into(version.pa);
   // clang-format on
   schema_version = ((((std::string{} += version.ma) += "#") += version.mi) += "#") += version.pa;

   sql << "select height,hash from top_block_info", into(top_block_height), into(top_block_hash);

   // Three different ways to obtain total number of transactions.
   sql << "select count(distinct hash) from tx_positions", into(total_transactions_count);
   unsigned long long counter;
   sql << "select count(*) from tx_positions where asset_id is null", into(counter);
   assert(total_transactions_count == counter);
   sql << "select count(*) from tx_status_by_hash where status = true", into(counter);
   assert(total_transactions_count == counter);
   ...
   for (auto& r : rs) {
      ...
      assert(it_dom != end(domains));
      auto& dom = *it_dom;
      auto [it_ap, inserted] = dom.assets_precision.insert(AssetPrecision{asset_id, (long double)precision});
      assert(inserted);
   }
   ...
   for (auto& r : rs) {
      ...
      auto& acc = find_account_by_id(std::move(account_id));
      auto [it_as, inserted] = acc.signatories.insert(tolower(std::move(public_key)));
   ...
   for (auto& r : rs) {
      ...
      assert(inserted && "account_has_roles <acc@dom,role> => domains[id].accounts[id].roles");
   }
   ...
   for (auto& r : rs) {
      ...
      if (!inserted) {
         fmt::print("--gp {} already exist in acc {}: {}\n", gp, permitter_account_id, *it_gp);
         cout << "acc.grantable_permissions:[" << acc.grantable_permissions << "]" << endl;
      }
...
try {
   ...
   cout << "in " << duration_cast<milliseconds>(system_clock::now() - start_rd).count() << "ms" << endl;
   std::ofstream(std::filesystem::current_path() / "rockdb.wsv") << "wsv_rocks:\n" << wsv_rocks << endl;
   // cout << "-----------------------" << endl;
   ...
   cout << "in " << duration_cast<milliseconds>(system_clock::now() - start_pg).count() << "ms" << endl;
   std::ofstream(std::filesystem::current_path() / "postgres.wsv") << "wsv_postgres:\n" << wsv_postgres << endl;
