...
CommandResult RocksDbCommandExecutor::operator()(
  ...
  forPeerAddress<kDbOperation::kGet, kDbEntry::kMustNotExist>(
      common, peer.pubkey(), [](auto /*pubkey*/, auto /*opt_address*/) {});

  /// Store address
  common.valueBuffer().assign(peer.address());
  forPeerAddress<kDbOperation::kPut>(
      common, peer.pubkey(), [](auto /*pubkey*/, auto /*opt_address*/) {});

  /// Store TLS if present
  if (peer.tlsCertificate().has_value()) {
    common.valueBuffer().assign(peer.tlsCertificate().value());
    forPeerTLS<kDbOperation::kPut>(
