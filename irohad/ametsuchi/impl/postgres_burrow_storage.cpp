...
Result<void, std::string> PostgresBurrowStorage::updateAccount(
  ...
  try {
    int check = 0;
    *sql() << "insert into burrow_account_data (address, data) "
...
Result<void, std::string> PostgresBurrowStorage::removeAccount(
  ...
  try {
    int check = 0;
    *sql() << "delete from burrow_account_key_value "
              "where address = lower(:address); "
...
PostgresBurrowStorage::getStorage(std::string_view address,
  ...
  try {
    ...
    *sql() << "select value from burrow_account_key_value "
...
Result<void, std::string> PostgresBurrowStorage::setStorage(
  ...
  try {
    int check = 0;
    *sql() << "insert into burrow_account_key_value (address, key, value) "
...
Result<void, std::string> PostgresBurrowStorage::storeLog(
  ...
  try {
    ...
    if (call_id_cache_) {
      *sql() << "insert into burrow_tx_logs (call_id, address, data) "
    ...
    if (not topics.empty()) {
      ...
      *sql() << "insert into burrow_tx_logs_topics (topic, log_idx) "
