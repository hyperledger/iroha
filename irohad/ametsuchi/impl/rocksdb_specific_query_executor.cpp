...
QueryExecutorResult RocksDbSpecificQueryExecutor::operator()(
    ...
    const shared_model::interface::types::HashType &query_hash,
    shared_model::interface::RolePermissionSet const &creator_permissions) {
  checkPermissions(creator_permissions, Role::kGetPeers);
  RocksDbCommon common(db_context_);
  ...
  for (auto &peer : peers)
    if (auto opt_tls = forPeerTLS<kDbOperation::kGet, kDbEntry::kCanExist>(
            common, peer->pubkey(), [&](auto /*pubkey*/, auto opt_tls) {
              return opt_tls;
            })) {
      std::reinterpret_pointer_cast<shared_model::plain::Peer>(peer)
          ->setTlsCertificate(*opt_tls);
    }

  return query_response_factory_->createPeersResponse(std::move(peers),
                                                      query_hash);
}
