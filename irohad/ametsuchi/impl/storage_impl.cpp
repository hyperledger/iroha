...
iroha::expected::Result<void, std::string> StorageImpl::resetWsv() {
...
iroha::expected::Result<ConnectionContext, std::string>
...
boost::optional<std::unique_ptr<iroha::LedgerState>> StorageImpl::commit(
...
StorageImpl::commitPrepared(
  ...
  try {
    std::shared_lock<std::shared_timed_mutex> lock(drop_mutex_);
    if (not connection_) {
      log_->info("commitPrepared: connection to database is not initialised");
      return boost::none;
    }
    soci::session sql(*connection_);
    ...
    log_->warn(
        "failed to apply prepared block {}: {}", block->hash().hex(), e.what());
    return boost::none;
  }
}
  ...
  if (block_is_prepared_) {
    PgConnectionInit::rollbackPrepared(session, prepared_block_name_)
        .match([this](auto &&v) { block_is_prepared_ = false; },
               [this](auto &&e) {
                 log_->info("Block rollback  error: {}", std::move(e.error));
               });
  }
}
