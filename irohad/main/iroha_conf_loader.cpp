...
inline bool JsonDeserializerImpl::loadInto(
  ...
  return getDictChild(config_members::PythonPaths).loadInto(dest.python_paths)
      and getDictChild(config_members::ModuleName).loadInto(dest.module_name)
      and getDictChild(config_members::InitArgument)
              .loadInto(dest.initialization_argument);
}
...
inline bool JsonDeserializerImpl::loadInto(
  ...
  std::string type =
      getDictChild(config_members::Type).deserialize<std::string>();
  assert_fatal(type == config_members::Python,
               fmt::format("Unknown data model module type: '{}'", type));
  dest = IrohadConfig::DataModelModule::Python{};
  return loadInto(std::get<IrohadConfig::DataModelModule::Python>(dest));
}

template <>
inline bool JsonDeserializerImpl::loadInto(
    IrohadConfig::DataModelModule &dest) {
  return loadInto(dest.module);
}
...
inline bool JsonDeserializerImpl::loadInto(IrohadConfig &dest) {
  return getDictChild(config_members::BlockStorePath)
             .loadInto(dest.block_store_path)
      and getDictChild(config_members::ToriiPort).loadInto(dest.torii_port)
      and getDictChild(config_members::ToriiTlsParams)
              .loadInto(dest.torii_tls_params)
      and getDictChild(config_members::InterPeerTls)
              .loadInto(dest.inter_peer_tls)
      and getDictChild(config_members::InternalPort)
              .loadInto(dest.internal_port)
      and getDictChild(config_members::PgOpt).loadInto(dest.pg_opt)
      and getDictChild(config_members::DbConfig).loadInto(dest.database_config)
      and getDictChild(config_members::MaxProposalSize)
              .loadInto(dest.max_proposal_size)
      and getDictChild(config_members::ProposalDelay)
              .loadInto(dest.proposal_delay)
      and getDictChild(config_members::VoteDelay).loadInto(dest.vote_delay)
      and getDictChild(config_members::MstSupport).loadInto(dest.mst_support)
      and getDictChild(config_members::MstExpirationTime)
              .loadInto(dest.mst_expiration_time)
      and getDictChild(config_members::MaxRoundsDelay)
              .loadInto(dest.max_round_delay_ms)
      and getDictChild(config_members::StaleStreamMaxRounds)
              .loadInto(dest.stale_stream_max_rounds)
      and getDictChild(config_members::LogSection).loadInto(dest.logger_manager)
      and getDictChild(config_members::InitialPeers)
              .loadInto(dest.initial_peers)
      and getDictChild(config_members::UtilityService)
              .loadInto(dest.utility_service)
      and getDictChild(config_members::DataModelModules)
              .loadInto(dest.data_model_modules)
      and getDictChild(config_members::kCrypto).loadInto(dest.crypto);
}
