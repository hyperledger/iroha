...
pub enum GetConfiguration {
    ...
    ///
    /// To get the documentation on the [`Log::config::Configuration.max_level`]
    /// `curl -X GET -H 'content-type: application/json' http://127.0.0.1:8080/configuration -d '{"Docs" : ["log", "max_level"]}' -i`
    Docs(Vec<String>),
