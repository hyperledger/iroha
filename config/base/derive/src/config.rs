...
use syn::{
    ...
    Data, DataStruct, DeriveInput, Fields, Ident, Lit, LitStr, Meta, Token, Type, TypePath,
};

use super::utils;

pub(crate) struct EnvPrefix {
    _ident: Ident,
    _eq: Token![=],
...
impl Parse for EnvPrefix {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        Ok(Self {
            _ident: utils::parse_const_ident(input, attrs::ENV_PREFIX)?,
            _eq: input.parse()?,
            prefix: input.parse()?,
        })
    }
...
impl Parse for Inner {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        Ok(Self {
            _ident: utils::parse_const_ident(input, attrs::INNER)?,
        })
    }
}

pub(crate) struct SerdeAsStr {
    _ident: Ident,
}
...
impl Parse for SerdeAsStr {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        Ok(Self {
            _ident: utils::parse_const_ident(input, attrs::SERDE_AS_STR)?,
        })
    }
}
...
pub(crate) fn impl_load_env(
    ...
    let set_field = field_ty
        ...
        .zip(inner.iter())
        .zip(option.iter())
        .map(|((((set_field, l_value), field_env), &inner_thing), &is_option)| {
...
fn impl_get_recursive(
    ...
    let variants = field_idents
        ...
        .zip(option)
        .zip(lvalue.iter())
        .map(|(((ident, &inner_thing), &is_option), l_value)| {
...
pub(super) fn impl_configurable(ast: &DeriveInput) -> TokenStream {
    let name = &ast.ident;
    let prefix = ast
        .attrs
        .iter()
    ...
    let lvalue = field_ty
        ...
        .zip(field_ty.iter().map(utils::is_option_type))
        .zip(field_idents.iter());
    let lvalue_read = lvalue
        .clone()
        .map(|((is_arc_rwlock, _), ident)| {
            if is_arc_rwlock {
    ...
    let lvalue_write = lvalue
        .clone()
        .map(|((is_arc_rwlock, is_option), ident)| {
            if is_arc_rwlock {
    ...
    let get_recursive = impl_get_recursive(&field_idents, &inner, &option, &lvalue_read);
