    ...
    class RDBExecutorTest : public AmetsuchiTest {
     public:
      ...
      void SetUp() override {
        ...
        wsv_query = std::make_unique<RocksDBWsvQuery>(
            db_port, getTestLogger("WsvQuery"));
      ...
      std::vector<std::string> getRoles(std::string_view account_id) {
        auto const names = iroha::ametsuchi::staticSplitId<2>(account_id);
        ...
        return std::move(result).match(
            [](auto &&v) {
              return boost::make_optional(
                  std::shared_ptr<T>(std::move(v.value)));
            },
      ...
      getDomain(const shared_model::interface::types::DomainIdType &domain_id) {
        ametsuchi::RocksDbCommon common(tx_context_);
        if (auto result = forDomain(common, domain_id);
            expected::hasValue(result))
          return fromResult(
              factory->createDomain(domain_id,
      ...
      bool hasAccountGrantablePermission(
        ...
        ametsuchi::RocksDbCommon common(tx_context_);
        if (auto result = forGrantablePermissions<kDbOperation::kGet,
                                                  kDbEntry::kMustExist>(
      ...
      getAccountAsset(
        ...
        if (auto result =
                forAccountAsset<kDbOperation::kGet, kDbEntry::kMustExist>(
                    common, account, domain, asset_id);
            expected::hasValue(result))
          return fromResult(factory->createAccountAsset(
              account_id, asset_id, *result.assumeValue()));
        else
          return boost::none;
      }
