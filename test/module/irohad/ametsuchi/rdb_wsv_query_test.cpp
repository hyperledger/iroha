    ...
    class RdbWsvQueryTest : public AmetsuchiTest {
     public:
      void SetUp() override {
        ...
        command = std::make_unique<RocksDBWsvCommand>(std::make_shared<RocksDBContext>(db_port));
        query = std::make_unique<RocksDBWsvQuery>(std::make_shared<RocksDBContext>(db_port),
