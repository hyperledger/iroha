...
struct MSTNotificationsTest : public ::testing::Test {
  ...
  void SetUp() override {
	...
	mst_state_update_ = iroha::SubscriberCreator<bool, std::shared_ptr<shared_model::interface::TransactionBatch>>::template create<
      iroha::EventTypes::kOnMstStateUpdate>(
      iroha::SubscriptionEngineHandlers::kNotifications,
      [&](auto &, std::shared_ptr<shared_model::interface::TransactionBatch> batch) {
      	ASSERT_TRUE(batch);
      	event_updated_.push_back(batch);
      });
  mst_state_prepared_ = iroha::SubscriberCreator<bool, std::shared_ptr<shared_model::interface::TransactionBatch>>::template create<
      iroha::EventTypes::kOnMstPreparedBatches>(
      iroha::SubscriptionEngineHandlers::kNotifications,
      [&](auto &, std::shared_ptr<shared_model::interface::TransactionBatch> batch) {
      	ASSERT_TRUE(batch);
      	event_prepared_.push_back(batch);
      });
  mst_state_expired_ = iroha::SubscriberCreator<bool, std::shared_ptr<shared_model::interface::TransactionBatch>>::template create<
      iroha::EventTypes::kOnMstExpiredBatches>(
      iroha::SubscriptionEngineHandlers::kNotifications,
      [&](auto &, std::shared_ptr<shared_model::interface::TransactionBatch> batch) {
      	ASSERT_TRUE(batch);
