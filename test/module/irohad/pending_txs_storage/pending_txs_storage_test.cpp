 ...
 public:
  ...
  auto twoTransactionsBatch() {
    return addSignatures(
        ...
        0,
        makeSignature("1", "pub_key_1"_hex_pubkey));
  }
...
TEST_F(PendingTxsStorageFixture, SignaturesUpdate) {
  ...
  *state1 += transactions;
  transactions =
      addSignatures(transactions, 0, makeSignature("2", "pub_key_2"_hex_pubkey));
  *state2 += transactions;
...
TEST_F(PendingTxsStorageFixture, SeveralBatches) {
  ...
  auto batch3 = addSignatures(
      ...
      0,
      makeSignature("2", "pub_key_2"_hex_pubkey));
  *state += batch1;
  *state += batch2;
  *state += batch3;
...
TEST_F(PendingTxsStorageFixture, PreparedBatch) {
  ...
  *state += batch;
  ...
  batch = addSignatures(batch,
                        0,
                        makeSignature("2", "pub_key_2"_hex_pubkey),
                        makeSignature("3", "pub_key_3"_hex_pubkey));
