...
TEST(Hash, sha3_256_empty_text) {
  ...
  ASSERT_STREQ(sha3_256(makeByteRange("")).to_hexstring().c_str(), res.c_str());
}
...
TEST(Hash, sha3_512_empty_text) {
  ...
  ASSERT_STREQ(sha3_512(makeByteRange("")).to_hexstring().c_str(), res.c_str());
}

TEST(Hash, sha3_256_ASCII_text) {
  std::string res =
      "cb7c96616a2466df29a1edc2979ef5080945f92d1907c08a55b502eba063d638";
  const char *str_ = "Is the Order a distributed ledger?";
  ASSERT_STREQ(sha3_256(makeByteRange(str_)).to_hexstring().c_str(),
               res.c_str());
}
...
TEST(Hash, sha3_512_ASCII_text) {
  ...
  const char *str_ = "Is the Order a distributed ledger?";
  ASSERT_STREQ(sha3_512(makeByteRange(str_)).to_hexstring().c_str(),
               res.c_str());
}
...
TEST(Hash, sha3_256_JP_text_LOOP) {
  ...
  for (int i = 0; i < LOOP_N; i++) {
    EXPECT_STREQ(sha3_256(makeByteRange(str_)).to_hexstring().c_str(),
                 res.c_str());
  }
}
...
TEST(Hash, sha3_512_JP_text_LOOP) {
  ...
  for (int i = 0; i < LOOP_N; i++) {
    EXPECT_STREQ(sha3_512(makeByteRange(str_)).to_hexstring().c_str(),
                 res.c_str());
  }
}
