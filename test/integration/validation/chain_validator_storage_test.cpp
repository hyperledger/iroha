...
namespace iroha {
  ...
  class ChainValidatorStorageTest : public ametsuchi::AmetsuchiTest {
   public:
    ...
    auto generateAndApplyFirstBlock() {
      ...
      auto tx =
          completeTx(baseTx()
                         .addPeer("0.0.0.0:50541", Pk{signers.at(0)->publicKey()})
                         .addPeer("0.0.0.0:50542", Pk{signers.at(1)->publicKey()})
                         .addPeer("0.0.0.0:50543", Pk{signers.at(2)->publicKey()})
                         .addPeer("0.0.0.0:50544", Pk{signers.at(3)->publicKey()}));
  ...
  TEST_F(ChainValidatorStorageTest, PeerAdded) {
    ...
    auto add_peer = completeTx(baseTx().addPeer(
        "0.0.0.0:50545", PublicKeyHexStringView{signers.at(4)->publicKey()}));
