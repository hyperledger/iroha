...
<!-- FIXME: I don't like that this section suggests using docker config_samples for deployment -->
...
1. Prepare a deployment environment.
    ...
    # cp ./config_samples/peer/config.json deploy
    # cp ./config_samples/peer/genesis.json deploy
    ```
...
We provide a sample configuration for Docker in [`docker-compose.yml`](../config_samples/swarm/docker-compose.yml). We highly recommend that you adjust the `config.json` to include a set of new key pairs.
