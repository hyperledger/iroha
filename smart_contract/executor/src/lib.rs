...
impl DataModelBuilder {
    ...
    #[must_use]
    pub fn add_parameter<T: parameter::Parameter>(mut self) -> Self {
        <T as iroha_schema::IntoSchema>::update_schema_map(&mut self.schema);
        self.parameters.insert(<T as parameter::Parameter>::id());
        self
    }

    /// Remove a configuration parameter from the data model
    #[must_use]
    pub fn remove_parameter<T: parameter::Parameter>(mut self) -> Self {
        <T as iroha_schema::IntoSchema>::remove_from_schema(&mut self.schema);
        self.parameters.remove(&<T as parameter::Parameter>::id());
        self
    }
