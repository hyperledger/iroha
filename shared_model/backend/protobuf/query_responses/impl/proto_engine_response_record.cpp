...
EngineReceipt::EngineReceipt(const TransportType &proto)
    ...
    , response_data_((proto.has_call_result() && !proto.call_result().result_data().empty()) ?
            ...
            std::nullopt)
    , call_result_(proto.has_call_result()
                    ? std::optional<shared_model::interface::EngineReceipt::CallResult>({proto.call_result().callee(), response_data_})
                    : std::nullopt)
    , contact_address_(proto.opt_to_contract_address_case() == iroha::protocol::EngineReceipt::kContractAddress
                    ? std::optional<shared_model::interface::types::EvmAddressHexString>(proto.contract_address())
                    : std::nullopt)
    {
        static_assert(offsetof(EngineReceipt, response_data_) < offsetof(EngineReceipt, call_result_),
            "Check ctor");
