...
impl Execute for data_model::Instruction {
    ...
    fn execute(&self) -> Self::Result {
        ...
        unsafe { host_execute_instruction(self as *const _ as usize) };
    }
}
impl Execute for data_model::QueryBox {
    ...
    fn execute(&self) -> Self::Result {
        ...
        unsafe { decode_with_length_prefix_from_raw(host_execute_query(self as *const _ as usize)) }
    }
}
...
pub(crate) mod host {
    ...
    extern "C" {
        ...
        pub(super) fn _iroha_wasm_fn(fn_id: u32, ptr: usize, len: usize) -> usize;
    }
}
...
mod tests {
    ...
    fn encode_query_result(res: Value) -> Vec<u8> {
        ...
        for (i, byte) in r.len().to_be_bytes().into_iter().enumerate() {
            r[i] = byte;
        }
