...
use tracing_subscriber::{Layer, fmt::MakeWriter, layer::SubscriberExt, registry::Registry, reload::Handle};

pub mod errors;
pub mod layer;
pub mod telemetry;

pub use crate::errors::ErrorLogging;
...
trait DumbTypingWorkaround {
	type L;

	fn reload() -> Result<(), tracing_subscriber::reload::Error> {
		todo!()
	}
}
...
fn add_bunyan<L: Layer<Registry> + Send + Sync + 'static>(
	configuration: &LoggerConfiguration,
	layer: L,
) -> Result<((SubstrateTelemetry, FutureTelemetry), Handle<L, Registry>)> {
	...
	if let Some(path) = configuration.log_file_path.clone() {
		...
		let subscriber = Registry::default()
			...
			.with(bunyan_layer);
		Ok((add_telemetry_and_set_default(configuration, subscriber)?, handle))
	} else {
