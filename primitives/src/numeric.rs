...
/// If more reach functionality is required (e.g. in smartcontract)
...
/// Decimal number constrained by [`NumericSpec`].
/// Should be used for storage only, all math operations should be performed on unconstrained decimal and then converted to constrained
#[derive(
    Serialize,
    Deserialize,
    Clone,
...
mod schema_ {
    use iroha_schema::{
        Declaration, Ident, IntoSchema, MetaMap, Metadata, NamedFieldsMeta, TypeId,
    };
    ...
    impl IntoSchema for Numeric {
        ...
        fn update_schema_map(metamap: &mut MetaMap) {
            if !metamap.contains_key::<Self>() {
                if !metamap.contains_key::<u128>() {
                    <u128 as iroha_schema::IntoSchema>::update_schema_map(metamap);
                }
...
mod ffi {
    ...
    iroha_ffi::ffi_type! {
        unsafe impl Transparent for Numeric {
            type Target = u128;
        }
    }

    iroha_ffi::ffi_type! {
        unsafe impl Transparent for NumericConstrained {
            type Target = Numeric;
        }
    }
}
