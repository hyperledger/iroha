...
node ('master') {
  ...
  def x64WinBuildScript = load '.jenkinsci-new/builders/x64-win-build-steps.groovy'

  // Define Workers
  x64LinuxWorker = new Worker(label: 'docker-build-agent', cpusAvailable: 8)
  x64MacWorker = new Worker(label: 'mac-4', cpusAvailable: 4)
  ...
  if(!mac_compiler_list.isEmpty()){
    ...
    if(specialBranch && build_type == 'Debug'){
      x64MacBuildSteps += [{x64BuildScript.buildSteps(parallelism==0 ?x64MacWorker.cpusAvailable : parallelism,
        mac_compiler_list, 'Release', false, false, testList, true, false, false, environmentList, r)}]
    }
    x64MacBuildPostSteps = new Builder.PostSteps(
      always: [{x64BuildScript.alwaysPostSteps(environmentList, r)}],
      success: [{x64BuildScript.successPostSteps(scmVars, packagePush, environmentList, r)}])
  }
