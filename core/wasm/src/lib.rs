...
unsafe extern "C" fn _iroha_wasm_fn(fn_id: u32, args_ptr: usize, args_len: usize) -> usize {
    let args_bytes = core::slice::from_raw_parts(args_ptr as *const u8, args_len);

    match fn_id {
        0 => {
            let domain_id = <Domain as Identifiable>::Id::decode(&mut &args_bytes[..]).unwrap();
            Domain::new(domain_id);
        }
        _ => panic!("unknown fn id"),
    }
    42
}
