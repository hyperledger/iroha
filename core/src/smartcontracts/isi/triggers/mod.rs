...
pub mod query {
    ...
    impl ValidQuery for FindTriggerById {
        ...
        fn execute(&self, wsv: &WorldStateView) -> Result<Self::Output, Error> {
            ...
            let instructions_opt = if let LoadedExecutable::Instructions(instructions) = executable
            {
                Some(instructions)
            } else {
                None
            };
            let action = Action::new(instructions_opt, repeats, technical_account, filter)
            ...
            let triggers = wsv
                ...
                .inspect_by_domain_id(domain_id, |trigger_id, action| {
                    ...
                    let instructions_opt =
                        if let LoadedExecutable::Instructions(instructions) = executable {
                            Some(instructions)
                        } else {
                            None
                        };
                    Trigger::new(
                        trigger_id.clone(),
                        Action::new(instructions_opt, repeats, technical_account, filter)
                            .with_metadata(metadata),
                    )
                });
