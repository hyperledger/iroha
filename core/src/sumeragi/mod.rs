...
use iroha_primitives::sync::{Mutex, MutexGuard};
use main_loop::State;
...
impl Sumeragi {
    ...
    pub fn update_metrics(&self) -> Result<()> {
        ...
        let mut last_guard = self.last_update_metrics_mutex.lock_mut();
    ...
    pub fn update_online_peers(&self, online_peers: Vec<PeerId>) {
        *self.internal.current_online_peers.lock_mut() = online_peers;
    }
