...
use crate::{
    block::{
        BlockHeader, ChainedBlock, EmptyChainHash, TriggerRecommendations, VersionedPendingBlock,
    },
    event::EventsSender,
    ...
    ContextHandler<RetrieveTransactions> for SumeragiWithFault<G, K, W, F>
{
    ...
    async fn handle(
        ...
        let trigger_recommendations = TriggerRecommendations::new();
    ...
    pub async fn start_genesis_round(
        ...
        if transactions.is_empty() {
            ...
            self.validate_and_publish_created_block(
                PendingBlock::new(transactions, TriggerRecommendations::new())
