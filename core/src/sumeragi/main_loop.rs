...
use iroha_primitives::{must_use::MustUse, sync::Mutex};
...
impl<F: FaultInjection> SumeragiWithFault<F> {
    ...
    pub fn zeroize(&self) {
        *self.latest_block_hash.lock_mut() = Hash::zeroed().typed()
    }
    ...
    F: FaultInjection,
{
    ...
    {
        let mut wsv = sumeragi.wsv.lock_mut();
        *wsv = state.wsv.clone();
    }
    ...
    *sumeragi.latest_block_hash.lock_mut() = state.latest_block_hash;
...
pub fn run<F>(
    ...
    F: FaultInjection,
{
    let mut incoming_message_receiver = sumeragi.message_receiver.lock_mut();
