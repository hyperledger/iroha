...
impl Sumeragi {
    ...
    fn commit_block(&mut self, block: impl Into<VersionedCommittedBlock>, new_wsv: WorldStateView) {
...
fn reset_state(
    ...
    latest_block: &VersionedCommittedBlock,
    ...
    if was_commit_or_view_change {
        ...
        *current_topology = Topology::recreate_topology(
            ...
            current_view_change_index
                .try_into()
                .expect("should not be large"),
            current_topology.sorted_peers.iter().cloned().collect(),
        );
...
enum BlockSyncOk {
    CommitBlock(VersionedCommittedBlock, WorldStateView),
    ReplaceTopBlock(VersionedCommittedBlock, WorldStateView),
}
