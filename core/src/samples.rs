//! This module contains the sample configurations used for testing throghout Iroha.
use iroha_crypto::{KeyPair, PublicKey};
use std::collections::HashSet;
use iroha_data_model::peer::{Id as PeerId};
use crate::{	
	config::Configuration, 
	torii::config::DEFAULT_TORII_P2P_ADDR,
...
pub fn get_trusted_peers(public_key: Option<&PublicKey>) -> HashSet<PeerId> {
    let mut trusted_peers: HashSet<PeerId> = [
        ...
        .map(|(a, k)| PeerId {
            address: a.to_string(),
            public_key: PublicKey::from_str(k).unwrap().clone(),
		})
        .collect();
	if let Some(pubkey) = public_key {
		trusted_peers.insert(PeerId {
			address: DEFAULT_TORII_P2P_ADDR.to_string(),
...
pub fn get_config(
    ...
    Configuration {
        ...
        sumeragi: SumeragiConfiguration {
            ...
            peer_id: PeerId::new(DEFAULT_TORII_P2P_ADDR, &public_key.clone()),
            block_time_ms: 1000,
        ...
        genesis: GenesisConfiguration {
            account_public_key: Some(public_key.clone()),
            account_private_key: Some(private_key.clone()),
