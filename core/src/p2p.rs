...
pub struct P2PSystem {
    ...
    pub(crate) connected_to_peers: Mutex<HashMap<PublicKey, OutgoingPeer>>,
    pub(crate) packet_buffers: Mutex<PacketBufferSet>,
}

pub(crate) struct PacketBufferSet {
    poll_network_index: u32,
    pub(crate) sumeragi_packets: Vec<crate::SumeragiPacket>,
    block_packets: Vec<crate::BlockSyncMessage>,
}

pub(crate) struct OutgoingPeer {
    stream: TcpStream,
...
impl P2PSystem {
    ...
    pub fn new(listen_addr: String, public_key: PublicKey) -> P2PSystem {
        P2PSystem {
            ...
            packet_buffers: Mutex::new(PacketBufferSet {
                poll_network_index: 0_u32,
                block_packets: Vec::new(),
                sumeragi_packets: Vec::new(),
            }),
        }
    }
    ...
    pub(crate) fn post_to_network(&self, message: &NetworkMessage, recipients: &[PublicKey]) {
        ...
        for public_key in recipients.iter() {
            if let Some(OutgoingPeer { stream, crypto, .. }) =
    ...
    pub(crate) fn poll_network_for_sumeragi_packet(&self) -> Option<crate::SumeragiPacket> {
        let PacketBufferSet {
            ref mut sumeragi_packets,
            ref mut block_packets,
    ...
    #[allow(clippy::unwrap_in_result)]
    pub(crate) fn poll_network_for_block_sync_message(&self) -> Option<crate::BlockSyncMessage> {
        let PacketBufferSet {
            ref mut sumeragi_packets,
            ref mut block_packets,
            ref mut poll_network_index,
    ...
    fn disconnect_peers(&self, to_disconnect_keys: Vec<PublicKey>) {
        ...
        for key in to_disconnect_keys {
            ...
            let OutgoingPeer { stream, .. } = connected_to_peers
                .remove(&key)
...
fn finish_connection(
    crypto: Cryptographer,
    mut stream: TcpStream,
    connected_to_peers: &mut HashMap<PublicKey, OutgoingPeer>,
    ...
    if let Ok(data) = crypto.decrypt(buf) {
        if let Ok(ConnectionCheck(size)) = Decode::decode(&mut data.as_slice()) {
            info!(size, "Established connection to peer {other_public_key}.");
