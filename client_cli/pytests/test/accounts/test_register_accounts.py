...
def test_register_account(
    ...
    with allure.step(
        ...
        registered = GIVEN_fake_name + '@' + GIVEN_new_one_existence_domain.name
    with allure.step(
...
def test_register_account_with_empty_name(
    ...
    with allure.step(
            ...
            f'in the "{GIVEN_new_one_existence_domain.name}" domain'):
        client_cli.register().account(account='', domain=GIVEN_new_one_existence_domain.name, key=GIVEN_public_key)
    with allure.step(
...
def test_register_account_with_existing_name(
    ...
    with allure.step(
            ...
            f'with the same name "{GIVEN_new_one_existence_domain.name}"  '
            f'in the "{GIVEN_new_one_existence_domain.name}" domain'):
        client_cli.register().account(account=GIVEN_new_one_existence_account.name, domain=GIVEN_new_one_existence_account.domain,
                                      key=GIVEN_new_one_existence_account.public_key)
    with allure.step(
            f'THEN client_cli should have the account error:  "{GIVEN_new_one_existence_domain.name}"'):
...
def test_register_account_with_invalid_character_in_key(
    ...
    with allure.step(
        ...
        client_cli.register().account(account=GIVEN_fake_name, domain=GIVEN_new_one_existence_domain.name,
...
def test_register_account_with_max_name(
    ...
    with allure.step(
        ...
        client_cli.register().account(account=GIVEN_127_lenght_name, domain=GIVEN_new_one_existence_domain.name,
                                      key=GIVEN_public_key)
        registered = GIVEN_127_lenght_name + '@' + GIVEN_new_one_existence_domain.name
    with allure.step(
            f'THEN Iroha should have the "{registered}" account'):
        iroha.should(have.account(registered))
...
def test_register_account_with_long_account_name(
    ...
    with allure.step(
        ...
        client_cli.register().account(account=GIVEN_129_lenght_name, domain=GIVEN_new_one_existence_domain.name,
                                      key=GIVEN_public_key)
    with allure.step(
