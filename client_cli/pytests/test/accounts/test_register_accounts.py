...
def test_register_account(GIVEN_fake_name, GIVEN_registered_domain, GIVEN_public_key):
    with allure.step(
...
def test_register_account_with_empty_name(GIVEN_registered_domain, GIVEN_public_key):
    with allure.step(
        f"WHEN client_cli tries to register an account with an empty name "
        f'in the "{GIVEN_registered_domain.name}" domain'
    ):
        client_cli.register().account(
            account="", domain=GIVEN_registered_domain.name, key=GIVEN_public_key
        )
    with allure.step(
...
def test_register_account_with_existing_name(
    GIVEN_registered_domain, GIVEN_public_key, GIVEN_registered_account
):
    with allure.step(
        ...
        client_cli.register().account(
            account=GIVEN_registered_account.name,
...
def test_register_account_with_invalid_character_in_key(
    GIVEN_fake_name, GIVEN_registered_domain, GIVEN_key_with_invalid_character_in_key
):
    with allure.step(
