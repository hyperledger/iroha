...
def test_mint_asset_for_account_in_same_domain(
    ...
    with allure.step(
            f'WHEN client_cli mint the asset "{GIVEN_existing_asset_definition_with_quantity_value_type.name}" '
            f'for the "{GIVEN_new_one_existing_account.name}" '
            f'in the "{GIVEN_existing_asset_definition_with_quantity_value_type.domain}" domain'
    ):
        client_cli.mint().asset(
                account=GIVEN_new_one_existing_account,
                asset_definition=GIVEN_existing_asset_definition_with_quantity_value_type,
                value_of_value_type=GIVEN_quantity_value
        )
    with allure.step(
            f'THEN "{GIVEN_new_one_existing_account}" account '
            f'should have the "{GIVEN_existing_asset_definition_with_quantity_value_type}" asset definition'):
        iroha.should(have.asset(f'{GIVEN_existing_asset_definition_with_quantity_value_type.name}##{GIVEN_new_one_existing_account}'))
