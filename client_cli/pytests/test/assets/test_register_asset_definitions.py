...
def test_register_asset_definition_with_quantity_value_type(
    GIVEN_fake_asset_name, GIVEN_registered_domain, GIVEN_quantity_value_type
):
    with allure.step(
...
def test_register_asset_definition_with_big_quantity_type(
    GIVEN_fake_asset_name, GIVEN_registered_domain, GIVEN_big_quantity_value_type
):
    with allure.step(
        f'WHEN client_cli registers the asset_definition "{GIVEN_fake_asset_name}" '
...
def test_register_asset_with_existing_name(
    ...
    with allure.step(
        ...
        f'with the same name "{GIVEN_registered_asset_definition_with_quantity_value_type.name}"'
        f'in the "{GIVEN_registered_asset_definition_with_quantity_value_type.domain}" domain'
    ):
        client_cli.register().asset().definition(
            asset=GIVEN_registered_asset_definition_with_quantity_value_type.name,
    ...
    with allure.step(
        ...
        f'{GIVEN_registered_asset_definition_with_quantity_value_type.__repr__()}"'
    ):
...
def test_register_asset_with_empty_name(GIVEN_registered_domain):
    with allure.step(
        ...
        client_cli.register().asset().definition(
            asset="", domain=GIVEN_registered_domain.name, value_type="Quantity"
        )
    with allure.step(
...
def test_register_asset_with_not_existing_domain(
    GIVEN_not_existing_name, GIVEN_quantity_value_type, GIVEN_fake_asset_name
):
    with allure.step(
