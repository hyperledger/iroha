...
def test_register_asset_definition_with_quantity_value_type(
    ...
    with allure.step(
        ...
        iroha.should(have.asset_definition(GIVEN_fake_asset_name + '#' + GIVEN_new_one_existence_domain.name))
...
def test_register_asset_with_existing_name(
    ...
    with allure.step(
            f'WHEN client_cli tries to register an asset definition with the same name "{GIVEN_existence_asset_definition_with_quantity_value_type.name}"'
            f'in the "{GIVEN_existence_asset_definition_with_quantity_value_type.domain}" domain'):
        client_cli.register().asset().definition(asset=GIVEN_existence_asset_definition_with_quantity_value_type.name,
                                                 domain=GIVEN_existence_asset_definition_with_quantity_value_type.domain,
                                                 value_type=GIVEN_existence_asset_definition_with_quantity_value_type.value_type)
    with allure.step(
            f'THEN client_cli should have the asset definition error: "{GIVEN_existence_asset_definition_with_quantity_value_type.__repr__()}"'):
...
def test_register_asset_with_empty_name(
    ...
    with allure.step(
            ...
            f'in the "{GIVEN_new_one_existence_domain.name}" domain'):
        client_cli.register().asset().definition(asset='', domain=GIVEN_new_one_existence_domain.name, value_type='Quantity')
    with allure.step(
