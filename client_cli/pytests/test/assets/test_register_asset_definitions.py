...
def test_register_asset_definition_with_quantity_value_type(
    ...
    with allure.step(
        ...
        iroha.should(have.asset_definition(GIVEN_fake_asset_name + '#' + GIVEN_registered_domain.name))
...
def test_register_asset_definition_with_too_long_name(
    ...
    with allure.step(
        ...
        client_cli.register().asset().definition(
            ...
            value_type=GIVEN_quantity_value_type)
    with allure.step(
            f'THEN Iroha should have the asset "{GIVEN_129_lenght_name}"'):
...
@allure.label('sdk_test_id', 'register_asset_definition_with_big_quantity_type')
def test_register_asset_definition_with_big_quantity_type(
        GIVEN_fake_asset_name,
        GIVEN_registered_domain,
        GIVEN_big_quantity_value_type):
    with allure.step(
            f'WHEN client_cli registers the asset_definition "{GIVEN_fake_asset_name}" '
            f'with "{GIVEN_big_quantity_value_type}" value type'
            f'in the "{GIVEN_registered_domain.name}" domain'):
        client_cli.register().asset().definition(
            asset=GIVEN_fake_asset_name,
            domain=GIVEN_registered_domain.name,
...
def test_register_asset_with_existing_name(
    ...
    with allure.step(
            f'WHEN client_cli tries to register an asset definition with the same name "{GIVEN_registered_asset_definition_with_quantity_value_type.name}"'
            f'in the "{GIVEN_registered_asset_definition_with_quantity_value_type.domain}" domain'):
        client_cli.register().asset().definition(asset=GIVEN_registered_asset_definition_with_quantity_value_type.name,
                                                 domain=GIVEN_registered_asset_definition_with_quantity_value_type.domain,
                                                 value_type=GIVEN_registered_asset_definition_with_quantity_value_type.value_type)
    with allure.step(
            f'THEN client_cli should have the asset definition error: "{GIVEN_registered_asset_definition_with_quantity_value_type.__repr__()}"'):
...
def test_register_asset_with_empty_name(
    ...
    with allure.step(
            ...
            f'in the "{GIVEN_registered_domain.name}" domain'):
        client_cli.register().asset().definition(asset='', domain=GIVEN_registered_domain.name, value_type='Quantity')
    with allure.step(
