...
def test_transfer_asset(
    ...
    with allure.step(f'WHEN {GIVEN_currently_authorized_account.name} transfers 1 Quantity'
                     ...
                     f'to {GIVEN_registered_account.name}'):
        client_cli.transfer(
    ...
    with allure.step(f'THEN {GIVEN_currently_authorized_account.name} has 1 Quantity '
        ...
        iroha.should(have.asset(
            f'{GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition.name}#{GIVEN_currently_authorized_account.domain}#{GIVEN_registered_account}'))
...
def test_transfer_with_insufficient_funds(
        ...
        GIVEN_currently_authorized_account,
        GIVEN_currently_account_quantity_with_two_quantity_of_asset):
    with allure.step(f'WHEN {GIVEN_currently_authorized_account.name} attempts to transfer more than available Quantity'
                     f'of {GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition.name}'
                     f'to {GIVEN_registered_account.name}'):
        client_cli.transfer(
            asset=GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition,
            source_account=GIVEN_currently_authorized_account,
            target_account=GIVEN_registered_account,
            quantity=str(int(GIVEN_currently_account_quantity_with_two_quantity_of_asset.value)+1))
    with allure.step(f'THEN {GIVEN_currently_authorized_account.name} still has the original Quantity '
                     f'of {GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition.name}'
                     f'AND {GIVEN_registered_account.name} does not receive any additional Quantity'):
