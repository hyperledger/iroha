...
def test_transfer_asset(
    ...
    with allure.step(
        f"WHEN {GIVEN_currently_authorized_account.name} transfers 1 Quantity"
    ...
    with allure.step(
        f"THEN {GIVEN_currently_authorized_account.name} has 1 Quantity "
        f"of {GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition.name}"
...
def test_transfer_with_insufficient_funds(
    ...
    with allure.step(
        f"WHEN {GIVEN_currently_authorized_account.name} attempts to transfer more than available Quantity"
        f"of {GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition.name}"
        f"to {GIVEN_registered_account.name}"
    ):
        client_cli.transfer(
    ...
    with allure.step(
        f"THEN {GIVEN_currently_authorized_account.name} still has the original Quantity "
        f"of {GIVEN_currently_account_quantity_with_two_quantity_of_asset.definition.name}"
