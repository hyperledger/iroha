...
On this step you will start and run the image in a container in an interactive mode. You can use any available tags, default one would be ``latest``, and developments tag is ``develop``. Note that you might need to perform some actions with ``sudo`` rights.
