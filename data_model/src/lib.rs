...
pub mod parameter {
    ...
    pub use self::model::*;
    use super::*;
    use crate::executor::ExecutorDataModelObject;
    ...
    mod model {
        ...
        #[display(fmt = "PARAMETER `{definition_id}`")]
        #[ffi_type]
    ...
    impl Parameter {
        /// Constructor
        pub fn new(definition_id: ParameterId, payload: impl IntoJsonString) -> Self {
            Self {
                definition_id,
    ...
    impl From<Name> for ParameterId {
        fn from(name: Name) -> Self {
            Self { name }
        }
    }

    impl From<ParameterId> for Name {
        fn from(value: ParameterId) -> Self {
            value.name
        }
    }

    impl ExecutorDataModelObject for Parameter {
        type DefinitionId = ParameterId;

        fn new(id: Self::DefinitionId, payload: JsonString) -> Self {
            Parameter::new(id, payload)
        }
...
mod model {
    ...
    #[derive(Default, Debug, Clone, Eq, Encode, Decode)]
    #[ffi_type(unsafe {robust})]
    #[repr(transparent)]
...
impl JsonString {
    /// Deserialize JSON into a type
    /// # Errors
    /// See [`serde_json::from_str`].
    pub fn deserialize_to<'a, T>(&'a self) -> serde_json::Result<T>
    where
        T: Deserialize<'a>,
    {
