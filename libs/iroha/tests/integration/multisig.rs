...
use iroha_executor_custom_data_model::multisig::{MultisigArgs, MultisigRegisterArgs};
...
fn mutlisig() -> Result<()> {
    ...
    let wasm = iroha_wasm_builder::Builder::new("../../samples/triggers/multisig_register")
        .show_output()
        .build()?
        .optimize()?
        .into_bytes()?;
