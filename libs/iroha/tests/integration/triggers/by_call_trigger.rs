...
use iroha_executor_custom_data_model::mint_rose_args::MintRoseArgs;
...
fn trigger_in_genesis_using_base64() -> Result<()> {
    ...
    let wasm = iroha_wasm_builder::Builder::new("../../samples/triggers/mint_rose_trigger")
        .show_output()
        .build()?
        .optimize()?
        .into_bytes()?;
...
fn call_execute_trigger_with_args() -> Result<()> {
    ...
    let trigger_id = TriggerId::from_str(TRIGGER_NAME)?;
    let wasm = iroha_wasm_builder::Builder::new("../../samples/triggers/mint_rose_trigger_args")
        .show_output()
        .build()?
        .optimize()?
        .into_bytes()?;
