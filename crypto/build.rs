...
fn main() {
    ...
    if ffi_import && ffi_export {
        eprintln!("cargo:warning=Features `ffi-export` and `ffi-import` are mutually exclusive");
        eprintln!("cargo:warning=When both active, `ffi-import` feature takes precedence");
    }
}
