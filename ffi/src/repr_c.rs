...
use core::{hint::unreachable_unchecked, mem::ManuallyDrop, ptr::addr_of_mut};

use crate::{
    ir::{Ir, Opaque, Robust, Transmute, Transparent},
    slice::{OutBoxedSlice, SliceMut, SliceRef},
    FfiConvert, FfiOutPtr, FfiOutPtrRead, FfiOutPtrWrite, FfiReturn, FfiType, FfiWrapperOutput,
    LocalRef, LocalSlice, ReprC, Result,
};
...
/// The trait is used to replace the output type of the wrapper function generated by [`ffi_import`].
///
/// Most notably, the necessity to replace the output type for a wrapper function arises when the wrapper function returns:
/// - a reference that uses store during conversion (i.e. that are cloned)
/// - an opaque type reference
///
/// If a type also implements [`Ir`], i.e. has a defined internal representation, a blanket
/// implementation of [`FfiWrapperOutput`] will be provided.
pub trait CWrapperOutput<S>: COutPtr<S> {
...
/// # Safety
///
/// Type must not make use of the store during conversion via [`CTypeConvert::into_repr_c`] or [`CTypeConvert::try_from_repr_c`]
// TODO: this makes sense NonLocal<S: Cloned>?
...
impl<'itm, R: NonLocal<S> + 'itm, S> CType<&'itm S> for LocalRef<'itm, R>
where
    &'itm R: CType<&'itm S>,
{
    type ReprC = <&'itm R as CType<&'itm S>>::ReprC;
}
impl<'itm, R: NonLocal<S> + 'itm, S> COutPtr<&'itm S> for LocalRef<'itm, R>
where
    &'itm R: COutPtr<&'itm S>,
{
...
impl<'itm, R: NonLocal<S> + 'itm, S> CType<&'itm [S]> for LocalSlice<'itm, R>
where
    &'itm [R]: CType<&'itm [S]>,
{
    type ReprC = <&'itm [R] as CType<&'itm [S]>>::ReprC;
}
impl<'itm, R: NonLocal<S> + 'itm, S> COutPtr<&'itm [S]> for LocalSlice<'itm, R>
where
    &'itm [R]: COutPtr<&'itm [S]>,
{
    type OutPtr = <&'itm [R] as COutPtr<&'itm [S]>>::OutPtr;
...
impl<'itm, R: NonLocal<S>, S: Cloned> CWrapperOutput<&'itm S> for &'itm R {
    type ReturnType = LocalRef<'itm, R>;
}
    ...
    COutPtrRead<&'itm S> for LocalRef<'itm, R>
{
    unsafe fn try_read_out(out_ptr: Self::OutPtr) -> Result<Self> {
        ...
        let store_borrow = &mut *addr_of_mut!(store);
        let item = <R as CTypeConvert<S, _>>::try_from_repr_c(out_ptr, store_borrow)?;
...
impl<R: NonLocal<S>, S: Cloned> CWrapperOutput<Box<S>> for Box<R> {
...
impl<'itm, R: NonLocal<S>, S: Cloned> CWrapperOutput<&'itm [S]> for &'itm [R] {
    type ReturnType = LocalSlice<'itm, R>;
}
impl<R: NonLocal<S>, S: Cloned> COutPtr<&[S]> for &[R] {
...
impl<R: NonLocal<S>, S: Cloned> CWrapperOutput<Vec<S>> for Vec<R> {
    type ReturnType = Self;
}
    ...
    COutPtrRead<Vec<S>> for Vec<R>
{
    unsafe fn try_read_out(out_ptr: Self::OutPtr) -> Result<Self> {
        ...
        // Correct as long as `R::try_from_repr_c` doesn't return a reference to the store (`R: NonLocal`)
        let store_borrow = &mut *addr_of_mut!(store);
        let res = <Vec<R>>::try_from_repr_c(slice, store_borrow);
...
impl<R: NonLocal<S>, S: Cloned, const N: usize> CWrapperOutput<[S; N]> for [R; N] {
    type ReturnType = Self;
}
impl<R: CType<S>, S: Cloned, const N: usize> COutPtr<[S; N]> for [R; N] {
    ...
    [R::FfiStore; N]: Default,
{
    unsafe fn write_out(self, out_ptr: *mut Self::OutPtr) {
        ...
        // Correct as long as `R::into_repr_c` doesn't return a reference to the store (`R: NonLocal`)
        let store_borrow = &mut *addr_of_mut!(store);
        let item = CTypeConvert::<[S; N], _>::into_repr_c(self, store_borrow);
...
impl<R: ReprC> CWrapperOutput<Robust> for R {
    type ReturnType = Self;
}
...
impl<R: ReprC + Default> CWrapperOutput<Box<Robust>> for Box<R> {
    type ReturnType = Self;
}
impl<R: ReprC> COutPtr<Box<Robust>> for Box<R> {
    type OutPtr = R;
}
impl<R: ReprC + Default> COutPtrWrite<Box<Robust>> for Box<R> {
    unsafe fn write_out(self, out_ptr: *mut Self::OutPtr) {
...
impl<R: ReprC + Default> COutPtrRead<Box<Robust>> for Box<R> {
    unsafe fn try_read_out(out_ptr: Self::OutPtr) -> Result<Self> {
...
impl<R: ReprC> CWrapperOutput<&mut [Robust]> for &mut [R] {
    type ReturnType = Self;
}
...
impl<R: ReprC> CWrapperOutput<Vec<Robust>> for Vec<R> {
    type ReturnType = Self;
}
...
impl<R: ReprC, const N: usize> CWrapperOutput<[Robust; N]> for [R; N] {
    type ReturnType = Self;
}
impl<R: ReprC, const N: usize> COutPtr<[Robust; N]> for [R; N] {
    type OutPtr = Self::ReprC;
}
impl<R: ReprC, const N: usize> COutPtrWrite<[Robust; N]> for [R; N] {
    unsafe fn write_out(self, out_ptr: *mut Self::OutPtr) {
        write_non_local::<_, [Robust; N]>(self, out_ptr)
    }
}
impl<R: ReprC, const N: usize> COutPtrRead<[Robust; N]> for [R; N] {
    unsafe fn try_read_out(out_ptr: Self::OutPtr) -> Result<Self> {
        read_non_local::<_, [Robust; N]>(out_ptr)
    }
}
...
impl<R: Transmute> CWrapperOutput<Transparent> for R
where
    R::Target: FfiWrapperOutput,
{
    type ReturnType = <R::Target as FfiWrapperOutput>::ReturnType;
}
...
impl<R: Transmute> CWrapperOutput<Box<Transparent>> for Box<R>
where
    Box<R::Target>: FfiWrapperOutput,
{
    type ReturnType = <Box<R::Target> as FfiWrapperOutput>::ReturnType;
}
...
impl<'slice, R: Transmute> CWrapperOutput<&'slice [Transparent]> for &'slice [R]
where
    &'slice [R::Target]: FfiWrapperOutput,
{
    type ReturnType = <&'slice [R::Target] as FfiWrapperOutput>::ReturnType;
}
...
impl<'slice, R: Transmute> CWrapperOutput<&'slice mut [Transparent]> for &'slice mut [R]
where
    &'slice mut [R::Target]: FfiWrapperOutput,
{
    type ReturnType = <&'slice mut [R::Target] as FfiWrapperOutput>::ReturnType;
}
...
impl<R: Transmute> CWrapperOutput<Vec<Transparent>> for Vec<R>
where
    Vec<R::Target>: FfiWrapperOutput,
{
    type ReturnType = <Vec<R::Target> as FfiWrapperOutput>::ReturnType;
}
    ...
    [R::Target; N]: FfiConvert<'itm, C>,
{
    ...
    unsafe fn try_from_repr_c(source: C, store: &'itm mut Self::FfiStore) -> Result<Self> {
        FfiConvert::try_from_ffi(source, store).and_then(|arr| transmute_from_target(arr))
    }
}

impl<R: Transmute, const N: usize> CWrapperOutput<[Transparent; N]> for [R; N]
where
    [R::Target; N]: FfiWrapperOutput,
{
    type ReturnType = <[R::Target; N] as FfiWrapperOutput>::ReturnType;
}
...
impl<R: Transmute, const N: usize> COutPtrWrite<[Transparent; N]> for [R; N]
where
    [R::Target; N]: FfiOutPtrWrite,
{
    unsafe fn write_out(self, out_ptr: *mut Self::OutPtr) {
        FfiOutPtrWrite::write_out(transmute_into_target(self), out_ptr);
    }
}
impl<R: Transmute, const N: usize> COutPtrRead<[Transparent; N]> for [R; N]
where
    [R::Target; N]: FfiOutPtrRead,
{
    unsafe fn try_read_out(out_ptr: Self::OutPtr) -> Result<Self> {
        FfiOutPtrRead::try_read_out(out_ptr).and_then(|output| transmute_from_target(output))
    }
