...
use repr_c::{
    COutPtr, COutPtrRead, COutPtrWrite, CType, CTypeConvert, CWrapperOutput, Cloned, NonLocal,
};
...
pub trait FfiWrapperOutput: FfiOutPtr {
...
macro_rules! ffi_type {
    ...
    (unsafe impl $(<$($impl_generics: tt $(: $bounds: path)?),*>)? Transparent for $ty: ty[$target: ty] $(where $where_clause: tt )? validated with {$validity_fn: expr}) => {
        ...
        impl<$($($impl_generics $(: $bounds)?),*)?> $crate::ReturnTypeOf<$ty> for $target where $($where_clause)? {
            type Type = $ty;
        }
    };
}
...
/// Define the correct [`FfiWrapperOutput::ReturnType`] out of the given [`CWrapperOutput::ReturnType`].
/// The only situation when this is evident is when [`Ir::Type`] is set to [`Transparent`]
///
/// Example:
///
/// ```
...
/// type `FfiType`, macro expansion will produce the following impl of [`CWrapperOutput`] for `Example`:
///
/// impl CWrapperOutput for Example {
///     type ReturnType = u32;
/// }
///
/// which is corrected via [`ReturnTypeOf`] so that functions generated by [`ffi_import`] return [`Example`]
/// */
/// ```
///
pub trait ReturnTypeOf<T> {
    /// Correct return type of `T` in a function generated via [`ffi_import`]
    type Type;
}

impl<T> ReturnTypeOf<Self> for T {
    type Type = Self;
}
impl<'itm, T> ReturnTypeOf<&'itm T> for *const T {
    type Type = &'itm T;
}
impl<'itm, T> ReturnTypeOf<&'itm mut T> for *mut T {
    type Type = &'itm mut T;
}
impl<'itm, R, T> ReturnTypeOf<&'itm R> for LocalRef<'itm, T> {
    type Type = LocalRef<'itm, R>;
}
impl<'slice, R, T> ReturnTypeOf<&'slice [R]> for LocalSlice<'slice, T> {
...
impl<R: Ir + CWrapperOutput<R::Type>> FfiWrapperOutput for R
where
    R::ReturnType: ReturnTypeOf<Self>,
{
    type ReturnType = <R::ReturnType as ReturnTypeOf<Self>>::Type;
}
...
macro_rules! impl_tuple {
    ( ($( $ty:ident ),+ $(,)?) -> $ffi_ty:ident ) => {
        ...
        impl<$($ty),+> CWrapperOutput<Self> for ($($ty,)+) where Self: COutPtrRead<Self> {
