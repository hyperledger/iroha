...
test_private_key= '7e00405ece477bb6dd9b03a78eee4e708afc2f5bcdce399573a5958942f4a390'
...
def argument_encoding(arg) :
    ...
    encoded_argument = encoded_argument+arg.encode('utf8').hex().ljust(64,'0').upper()
...
def create_account(address) :
    ...
    params=params+argument_encoding('domain') #domain id
    params=params+argument_encoding(test_private_key) #  key
    tx = iroha.transaction([
        iroha.command('CallEngine', caller=ADMIN_ACCOUNT_ID, callee=address, input=params)
    ])
    IrohaCrypto.sign_transaction(tx, ADMIN_PRIVATE_KEY)
...
address = get_engine_receipts(hash)
hash = balance(address)
get_engine_receipts(hash)
add_asset(address)
hash = balance(address)
get_engine_receipts(hash)
transfer(address)
