...
def get_account_transactions(address, first_tx_height = None, last_tx_height = None):
    params = get_first_four_bytes_of_keccak(b'getAccountTransactions(string,string,string,string,string,string,string,string)')
    no_of_param = 8
    for x in range(no_of_param):
        params = params + left_padded_address_of_param(x, no_of_param)
    params = params + argument_encoding('admin@test')  # account id
    params = params + argument_encoding('10')  # page size
    params = params + argument_encoding('')  # first_tx_hash
    params = params + argument_encoding('')  # ordering
    params = params + argument_encoding('')  # first_tx_time
    params = params + argument_encoding('')  # last_tx_time
    params = params + argument_encoding(str(last_tx_height))  # first_tx_height
    params = params + argument_encoding(str(first_tx_height))  # last_tx_height
    tx = iroha.transaction(
        [
            iroha.command(
...
def get_account_asset_transactions(address, first_tx_time = None, last_tx_time = None):
    params = get_first_four_bytes_of_keccak(b'getAccountAssetTransactions(string,string,string,string,string,string,string,string,string)')
    no_of_param = 9
    for x in range(no_of_param):
        params = params + left_padded_address_of_param(x, no_of_param)
    ...
    params = params + argument_encoding('')  # first_tx_time
    params = params + argument_encoding('')  # last_tx_time
    params = params + argument_encoding('')  # first_tx_height
    params = params + argument_encoding('')  # last_tx_height
    tx = iroha.transaction(
        [
            iroha.command(
                "CallEngine", caller=ADMIN_ACCOUNT_ID, callee=address, input=params
...
def get_pending_transactions(address, first_tx_time, last_tx_time):
    params = get_first_four_bytes_of_keccak(b'getPendingTransactions(string,string,string,string,string)')
    no_of_param = 5
    for x in range(no_of_param):
        params = params + left_padded_address_of_param(x, no_of_param)
    params = params + argument_encoding('10')  # page size
    params = params + argument_encoding('')  # first_tx_hash
    params = params + argument_encoding('')  # ordering
    params = params + argument_encoding(str(first_tx_time))  # first_tx_time
    params = params + argument_encoding(str(last_tx_time))  # last_tx_time
    tx = iroha.transaction(
        [
            iroha.command(
                "CallEngine", caller=ADMIN_ACCOUNT_ID, callee=address, input=params
...
def get_transaction(address):
    params = get_first_four_bytes_of_keccak(b'getTransaction(string)')
    no_of_param = 1
    for x in range(no_of_param):
        params = params + left_padded_address_of_param(x, no_of_param)
    params = params + argument_encoding('67619a99012600432ac15225373d192dba4b6d5f47dbf89daa8ec9fd9bee8c48')  # transaction hash 
    tx = iroha.transaction(
        [
            iroha.command(
                "CallEngine", caller=ADMIN_ACCOUNT_ID, callee=address, input=params
